...
export default {
  ...
  methods: {
    ...
    // 创建一个 HTTP GET 请求
    // 用户输入的内容
    // let user_content = this.content.trim()
    let user_content = '热介质膨胀罐的作用'
    const options = {
      path: `/stream?req_question=${user_content}&temperature=0.01&top_p=0.1&max_length=8192&req_history=[]&experience=0`,
      method: 'GET'
    };
      ...
      const req = http.request(options, res => {
        ...
        // 设置编码为 'utf8'，这样 data 事件接收到的数据块将会是字符串
        res.setEncoding('utf8');

        // 监听 'data' 事件来接收数据
        res.on('data', chunk => {
          allChunkText += chunk
          let temp = allChunkText.split("data:")
        ...
        // 监听 'end' 事件来知道何时没有更多数据
        res.on('end', () => {
          console.log('没有更多数据。');
        });
      });

      // 监听 'error' 事件来处理请求过程中的错误
      req.on('error', e => {
